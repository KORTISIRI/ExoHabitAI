...
export default function PredictionPanel({ onResult, loading, setLoading }) {
  ...
  const handleSubmit = async e => {
    ...
    const payload = {
      ...
      // optional fields
      ...(form.pl_rade   && { pl_rade:   parseFloat(form.pl_rade)   }),
      ...(form.pl_bmasse && { pl_bmasse: parseFloat(form.pl_bmasse) }),
      ...(form.pl_eqt    && { pl_eqt:    parseFloat(form.pl_eqt)    }),
      ...(form.st_lum    && { st_lum:    parseFloat(form.st_lum)    }),
      ...(form.st_rad    && { st_rad:    parseFloat(form.st_rad)    }),
    };
    ...
    try {
      ...
      if (err instanceof ApiError && err.code === 'NETWORK_ERROR') {
        ...
        setGlobalError('⚠ ML model not loaded on the server. Run python ML_Model_Training.py first.');
      } else if (err instanceof ApiError) {
        const detail = err.details ? JSON.stringify(err.details).slice(0, 120) : '';
        setGlobalError(`Prediction failed: ${err.message}${detail ? ` — ${detail}` : ''}`);
      } else {
        setGlobalError(`Unexpected error: ${err.message}`);
      }
    } finally {
      setLoading(false);
    }
