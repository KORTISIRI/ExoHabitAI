...
import { predictBatchFile, predictBatchJson, ApiError } from '../api';
...
export default function BatchUploadPanel() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (inputMode === 'file') {
        data = await predictBatchFile(file);
      } else {
        const items = parseJSON(jsonText.trim());
        data = await predictBatchJson(items);
      }
      ...
      if (err instanceof ApiError && err.code === 'NETWORK_ERROR') {
        ...
        setError('⚠ ML model not loaded. Run python ML_Model_Training.py first, then restart the server.');
      } else if (err instanceof ApiError) {
        setError(`Batch prediction failed: ${err.message}`);
      } else {
            ...
            >
              <div className="batch-stats-row">
                <div className="batch-stat-card total">
                  <span className="bsc-val">{results.total_items ?? (results.prediction_count ?? 0) + (results.error_count ?? 0)}</span>
              ...
              <div className="batch-toolbar">
                <div className="batch-filter-tabs">
                  {['all','habitable','non-habitable'].map(mode => (
                    ...
                    >
                      {mode === 'all' ? `All (${results.total_items ?? results.predictions?.length})`
